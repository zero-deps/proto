...
class testing extends AnyFreeSpec {
  ...
  object messages {
    ...
    object numscodec {
      implicit val vehicleCodec: MessageCodec[Vehicle] = {
        ...
        implicit val teleportCodec = classCodecNums[Teleport]("id"->2)((x: String) => Teleport.apply(x))
  ...
  object custom {
    val codecSynthetic1 = classCodecNums[SimpleClass[String]]("id"->1)((x: Int) => SimpleClass.init(x))
    val codecSynthetic2 = classCodecNums[SimpleClass[String]]("id"->1, "id2"->2)((x: Int, ys: List[String]) => SimpleClass.init2(x, ys))
    def codecSynthetic3[A:MessageCodec] = classCodecNums[SimpleClass[A]]("id"->1, "id2"->2)((x: Int, ys: List[A]) => SimpleClass.init3[A](x, ys))
