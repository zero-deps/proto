...
private class Impl(using Quotes) extends Ops with Encoders with Decoders:
  ...
  def generate[D: Type, E: Type](
    ...
    val moduleCommonValue = {
      if commonPursTypes.nonEmpty then {
        val code = commonPursTypes.flatMap(_.tmpl).mkString("\n")
        val is = List(
          if code.contains(" :: Eq ") then Some("Data.Eq" -> "class Eq") else None
        , if code.contains("Nothing") then Some("Data.Maybe" -> "Maybe(Nothing)") else None
        , if code.contains("Tuple ") then Some("Data.Tuple" -> "Tuple") else None
        , if code.contains("BigInt ") then Some("Proto.BigInt" -> "BigInt") else None
    ...
    val moduleEncodeValue = {
      ...
      val is = List(
        if code.contains("concatMap ") then Some("Data.Array" -> "concatMap") else None
      , if code.contains("Uint8Array") then Some("Proto.Uint8Array" -> "Uint8Array") else None
      , if code.contains("BigInt") then Some("Proto.BigInt" -> "BigInt") else None
      , if code.contains(" :: Eq ") then Some("Data.Eq" -> "class Eq") else None
      , if raw"\WMaybe ".r.findFirstIn(code).isDefined then Some("Data.Maybe" -> "Maybe(..)") else None
      // , if (code contains "Nothing") Some("Data.Maybe" -> "Maybe(Nothing)") else None
      , if code.contains("fromMaybe ") then Some("Data.Maybe" -> "fromMaybe") else None
      , if code.contains("Tuple ") then Some("Data.Tuple" -> "Tuple(Tuple)") else None
      , if code.contains("map ") then Some("Prelude" -> "map") else None
      , if code.contains(" $ ") then Some("Prelude" -> "($)") else None
      , if code.contains("Encode.") then Some("Proto.Encode as Encode" -> "") else None
      , if code.contains("length ") then Some("Proto.Uint8Array" -> "length") else None
      , if code.contains("concatAll ") then Some("Proto.Uint8Array" -> "concatAll") else None
      , if code.contains("fromArray ") then Some("Proto.Uint8Array" -> "fromArray") else None
      ...
      s"""|module $moduleEncode
          ...
          |
          |$is
          |${if commonPursTypes.nonEmpty then s"import $moduleCommon" else ""}
          |
          |$code""".stripMargin
    }
    ...
    val moduleDecodeValue = {
      ...
      val is = List(
        if code.contains("Loop ") || code.contains("Done ") then Some("Control.Monad.Rec.Class" -> "Step(Loop, Done)") else None
      , if code.contains("tailRecM3 ") then Some("Control.Monad.Rec.Class" -> "tailRecM3") else None
      , if code.contains("snoc ") then Some("Data.Array" -> "snoc") else None
      , if code.contains("Uint8Array ") then Some("Proto.Uint8Array" -> "Uint8Array") else None
      , if code.contains("BigInt") then Some("Proto.BigInt" -> "BigInt") else None
      , if code.contains("Left ") then Some("Data.Either" -> "Either(Left)") else None
      , if code.contains(" :: Eq ") then Some("Data.Eq" -> "class Eq") else None
      , if code.contains("zshr") then Some("Data.Int.Bits" -> "zshr") else None
      , if code.contains(" .&. ") then Some("Data.Int.Bits" -> "(.&.)") else None
      , if code.contains("Just ") || code.contains("Nothing") then Some("Data.Maybe" -> "Maybe(Just, Nothing)") else None
      , if code.contains("fromMaybe ") then Some("Data.Maybe" -> "fromMaybe") else None
      , if code.contains("Tuple ") then Some("Data.Tuple" -> "Tuple(Tuple)") else None
      , if code.contains("Unit") then Some("Data.Unit" -> "Unit") else None
      , if code.contains("unit") then Some("Data.Unit" -> "unit") else None
      , if code.contains("map ") then Some("Prelude" -> "map") else None
      , if code.contains(" do") then Some("Prelude" -> "bind") else None
      , if code.contains("pure ") then Some("Prelude" -> "pure") else None
      , if code.contains(" $ ") then Some("Prelude" -> "($)") else None
      , if code.contains(" + ") then Some("Prelude" -> "(+)") else None
      , if code.contains(" < ") then Some("Prelude" -> "(<)") else None
      , if code.contains(" <<< ") then Some("Prelude" -> "(<<<)") else None
      , if code.contains("Decode.") then Some("Proto.Decode as Decode" -> "") else None
      ).flatten.groupMapReduce(_._1)(_._2)(_ + ", " + _).map(x => "import " + x._1 + Some(x._2).filter(_.nonEmpty).map(" ("+_+")").getOrElse("")).to(List).sorted.mkString("\n")
