...
import proto.{encode}
import proto.{caseCodecIdx, caseCodecAuto, sealedTraitCodecAuto}

class PurescriptSpec extends AnyFreeSpec with Matchers {
  "generate" - {
    "src" in {
      val res = proto.purs.generate[Push, Pull](moduleEncode="Pull", moduleDecode="Push", moduleCommon="Common")
      res.purs.foreach{ case (filename, content) =>
        io.writeToFile(s"purs/test/src/$filename.purs", content)
      }
    }
    "test" in {
      val res = proto.purs.generate[TestSchema, TestSchema](moduleEncode="SchemaPull", moduleDecode="SchemaPush", moduleCommon="SchemaCommon")
      res.purs.foreach{ case (filename, content) =>
